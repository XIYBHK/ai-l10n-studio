...
mod tests {
    ...
    fn test_simple_phrases_should_reject_length() {
        // 长度超过35字符的应该拒绝
        assert!(!is_simple_phrase("This is a very long text that exceeds the 35 character limit"), 
                "应该拒绝: 超过35字符");
    }
    ...
    fn test_simple_phrases_should_reject_sentence_endings() {
        // 包含句子标点的应该拒绝
        assert!(!is_simple_phrase("This is a sentence. Another one"), "应该拒绝: 包含句号");
    ...
    fn test_simple_phrases_should_reject_too_many_words() {
        // 单词数超过5个的应该拒绝
        assert!(!is_simple_phrase("One Two Three Four Five Six"), 
                "应该拒绝: 超过5个单词");
    }
    ...
    fn test_simple_phrases_should_reject_placeholders() {
        // 包含占位符的应该拒绝
        assert!(!is_simple_phrase("Text with {0} placeholder"), "应该拒绝: 包含占位符 {{0}}");
        assert!(!is_simple_phrase("Another {1} test"), "应该拒绝: 包含占位符 {{1}}");
        assert!(!is_simple_phrase("Third {2} case"), "应该拒绝: 包含占位符 {{2}}");
    }
    ...
    fn test_simple_phrases_should_reject_escape_characters() {
        ...
        assert!(!is_simple_phrase("Text with\\rcarriage"), "应该拒绝: 包含 \\r");
    }
    
    #[test]
    fn test_simple_phrases_should_reject_special_symbols() {
        // 包含特殊符号的应该拒绝
        assert!(!is_simple_phrase("Text (with parentheses)"), "应该拒绝: 包含括号");
        assert!(!is_simple_phrase("Text [with brackets]"), "应该拒绝: 包含方括号");
    ...
    fn test_simple_phrases_should_reject_question_starters() {
        // 疑问句开头的应该拒绝
        assert!(!is_simple_phrase("Whether to sort"), "应该拒绝: Whether 开头");
    ...
    fn test_simple_phrases_should_reject_preposition_phrases() {
        // 介词短语应该拒绝
        assert!(!is_simple_phrase("for the purpose"), "应该拒绝: 'for ' 短语");
        assert!(!is_simple_phrase("in the middle"), "应该拒绝: 'in the ' 短语");
        assert!(!is_simple_phrase("on the table"), "应该拒绝: 'on the ' 短语");
    }
    ...
    fn test_simple_phrases_should_reject_descriptive_words() {
        ...
        assert!(!is_simple_phrase("Some examples"), "应该拒绝: 包含 examples");
    }
    ...
    fn test_edge_cases() {
        ...
        assert!(is_simple_phrase("One Two Three Four Five"), "应该接受: 刚好5个单词");
        assert!(!is_simple_phrase("One Two Three Four Five Six"), "应该拒绝: 超过5个单词");
    }
}
