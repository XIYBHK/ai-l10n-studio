...
export const configCommands = {
  ...
  async update(config: any) {
    return invoke<void>(COMMANDS.CONFIG_UPDATE, { config }, {
      errorMessage: '更新配置失败',
    });
  },

  async validate(config: any) {
    return invoke<boolean>(COMMANDS.CONFIG_VALIDATE, { config }, {
      errorMessage: '配置验证失败',
    });
  },
};
...
export const aiConfigCommands = {
  ...
  async setActive(id: string) {
    return invoke<void>(COMMANDS.AI_CONFIG_SET_ACTIVE, { id }, {
      errorMessage: '设置活动AI配置失败',
    });
  },

  async add(config: AIConfig) {
    return invoke<string>(COMMANDS.AI_CONFIG_ADD, { config }, {
      errorMessage: '添加AI配置失败',
    });
  },

  async update(id: string, config: AIConfig) {
    return invoke<void>(COMMANDS.AI_CONFIG_UPDATE, { id, config }, {
...
export const termLibraryCommands = {
  ...
  async removeTerm(source: string) {
    return invoke<void>(COMMANDS.TERM_LIBRARY_REMOVE, { source }, {
      errorMessage: '删除术语失败',
    });
  },

  async generateStyleSummary(apiKey: string) {
    return invoke<string>(COMMANDS.TERM_LIBRARY_GENERATE_STYLE, { apiKey }, {
...
export const translationMemoryCommands = {
  ...
  async save(memory: any) {
    return invoke<void>(COMMANDS.TM_SAVE, { memory }, {
...
export const poFileCommands = {
  async parse(filePath: string) {
    return invoke<POEntry[]>(COMMANDS.PO_PARSE, { filePath }, {
      errorMessage: '解析 PO 文件失败',
    });
  },

  async save(filePath: string, entries: POEntry[]) {
    return invoke<void>(COMMANDS.PO_SAVE, { filePath, entries }, {
...
export const fileFormatCommands = {
  async detect(filePath: string) {
    return invoke<string>(COMMANDS.FILE_FORMAT_DETECT, { filePath }, {
      errorMessage: '检测文件格式失败',
    });
  },

  async getMetadata(filePath: string) {
    return invoke<any>(COMMANDS.FILE_METADATA_GET, { filePath }, {
...
export const translatorCommands = {
  async translateEntry(text: string, targetLanguage?: string) {
    return invoke<string>(COMMANDS.TRANSLATE_ENTRY, {
      text,
  ...
  async batchTranslate(
    ...
    return invoke<POEntry[]>(COMMANDS.TRANSLATE_BATCH, {
      entries,
      targetLanguage: targetLanguage || null,
    }, {
      errorMessage: '批量翻译失败',
  ...
  async contextualRefine(requests: ContextualRefineRequest[], targetLanguage: string) {
    return invoke<string[]>(COMMANDS.CONTEXTUAL_REFINE, {
      requests,
      targetLanguage,
    }, {
