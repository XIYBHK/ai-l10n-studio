...
export const useNotification = (): NotificationAPI => {
  ...
  const send = useCallback(async (options: NotificationOptions) => {
    if (!isEnabled) {
      log.info('通知已禁用，跳过发送');
      return;
    }
  ...
  const success = useCallback(async (title: string, body: string) => {
    await send({
      title: `✅ ${title}`,
      body,
    });
  ...
  const error = useCallback(async (title: string, body: string) => {
    await send({
      title: `❌ ${title}`,
      body,
    });
  ...
  const info = useCallback(async (title: string, body: string) => {
    await send({
      title: `ℹ️ ${title}`,
      body,
    });
  ...
  const warning = useCallback(async (title: string, body: string) => {
    await send({
      title: `⚠️ ${title}`,
      body,
    });
