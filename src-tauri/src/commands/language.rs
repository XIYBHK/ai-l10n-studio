...
pub fn get_default_target_lang(source_lang_code: String) -> Result<LanguageInfo, String> {
    crate::app_log!("🔄 [语言检测] 获取默认目标语言，源语言代码: {}", source_lang_code);
    
    let source_lang = Language::from_code(&source_lang_code)
        .ok_or_else(|| {
    ...
    crate::app_log!("✅ [语言检测] 推荐目标语言: {} ({})", 
        target_info.display_name, target_info.code);
