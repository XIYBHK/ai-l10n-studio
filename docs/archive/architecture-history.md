...
> **当前参考**：阅读 "2026-05 update" section 了解当前架构状态。
> **历史 section**（"2025-11 重大性能重构" 及之后）保留原貌作为**重构历史参考**，含旧风格标记和 emoji，仅用于追溯演进脉络，不代表当前规范。
> **无 emoji 原则**：仅适用于当前代码、注释、新增文档；已存在的历史记录段落按"原貌保留"。

---

## Architecture Overview (2026-05 update)

### Core Stack

**Frontend**: React 19 + TypeScript + Ant Design 6 + Zustand 5 + SWR + @tanstack/react-virtual + i18next
...
### 2026-05 Frontend Audit Response (P0 → P2)

Following a comprehensive frontend review, the following changes shipped:

**P0 Robustness**:
- Fixed stable-closure bug in `useTranslationFlow` — destructured `useChannelTranslation` return to avoid recreating `cancelTranslation` on every render
- Added `isActive` race-condition guards to async `listen()` setup in `useTranslationMemory` and `useTermLibrary` (pattern matches `useTranslationFlow`)
- Migrated **60+ hardcoded Chinese strings** in `message.*()` / JSX to i18n, across 11 files; i18n namespaces grew from 8 to 14 (`app`, `messages`, `errors`, `errorBoundary`, `emptyState`, `devTools` added)

**P1 Architecture**:
- `AIWorkspace.tsx`: 710 → 178 LOC, extracted 9 sub-components into `components/aiWorkspace/`
- `EntryList.tsx`: 799 → 251 LOC, extracted `VirtualizedColumn` / `StatusColumns` / `BatchActions` sub-components and `useEntrySelection` hook
- `EditorPane.tsx`: 331 → 220 LOC, extracted `useTermDetection` hook
- `MenuBar.tsx`: props reduced 14 → 9 (theme consumed via `useTheme`, source/target language pushed down to `useTranslationStore`)

**P2 Polish**:
- DevTools UI labels migrated to `devTools` i18n namespace (25 replacements)
- `EmptyState` uses runtime `t()` for config and default shortcuts
- `useDeferredValue` on `MemoryManager` search, `startTransition` on large `setEntries`
- Antd Modal's built-in focus trap is sufficient; custom `FocusTrap` is documented for non-modal use only

**Visual P0 + P1**:
- Design token SSOT unified to `src/index.css` (removed conflicting `:root` definitions in `App.css`)
- `App.css` shrunk from 444 → 150 LOC (-66%) by removing duplicate animations, 9 emoji comments, and 20+ dead CSS classes
- Status color `needsReview` changed from Blue `#89b4fa` to Peach `#fab387` (matches "needs attention" semantics)
- `unsavedBadge` palette: Purple (1.2:1 contrast) → Peach + dark text (9:1 contrast, AAA)
- `StatCard` supports `size="large"` prop; `CumulativeStatsSection` uses it for the "total translated" primary metric with brand color
- New tokens: `--color-warning`, `--color-error`, `--color-flashGlow`, `--color-warningBg`, `--color-errorBg`, `--color-flashGlowSoft`
- All hardcoded `#ff4d4f` (Ant Design red) and `rgba(22, 119, 255, ...)` (Ant Design blue) replaced with Catppuccin tokens
